...
public:
	...
	/// \return
	///
    track_t CalcDistHist(const CRegion& reg, cv::Mat& hist, cv::Mat currFrame) const;
    ...
    void Update(const CRegion& region, bool dataCorrect, size_t max_trace_length, cv::Mat prevFrame, cv::Mat currFrame, int trajLen);
    void Update(const CRegion& region, const RegionEmbedding& regionEmbedding, bool dataCorrect, size_t max_trace_length, cv::Mat prevFrame, cv::Mat currFrame, int trajLen);
...
private:
    ...
    bool CheckStatic(int trajLen, cv::Mat currFrame, const CRegion& region);

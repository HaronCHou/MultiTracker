...
/// \return
///
track_t CTrack::CalcDistHist(const CRegion& reg, cv::Mat& hist, cv::Mat currFrame) const
{
	track_t res = 1;
    ...
    if (hist.empty())
	{
		...
		std::vector<cv::Mat> regROI = { currFrame(reg.m_brect) };
...
/// \return
///
bool CTrack::CheckStatic(int trajLen, cv::Mat currFrame, const CRegion& region)
{
    if (!trajLen || static_cast<int>(m_trace.size()) < trajLen)
    {
        m_isStatic = false;
        m_staticFrames = 0;
        m_staticFrame = cv::Mat();
    }
    else
    {
        track_t kx = 0;
...
/// \return
///
TrackingObject CTrack::ConstructObject() const//CTrack将m_trace给TrackingObject输出
{
                        ...
                        cv::Mat currFrame)
{
	...
	case tracking::TrackCSRT:
        ...
        if (!dataCorrect)
        {
            ...
            if (!m_tracker || m_tracker.empty())
            {
                        ...
                        lastRect.area() > 0)
                {
                    if (m_staticFrame.empty())
                        ...
                        m_tracker->init(cv::Mat(m_staticFrame, roiRect), lastRect);
#if 0
#ifndef SILENT_WORK
                    cv::Mat tmp;
                    if (m_staticFrame.empty())
                        tmp = cv::Mat(prevFrame, roiRect).getMat(cv::ACCESS_READ).clone();
                    else
                        tmp = cv::Mat(m_staticFrame, roiRect).getMat(cv::ACCESS_READ).clone();
            ...
            cv::Rect2d newRect;
            if (!inited && !m_tracker.empty() && m_tracker->update(cv::Mat(currFrame, roiRect), newRect))
            {
#if 0
#ifndef SILENT_WORK
                cv::Mat tmp2 = cv::Mat(currFrame, roiRect).getMat(cv::ACCESS_READ).clone();
